...
async function getRoutingTable(){
    ...
    if (response.status == 200){
        ...
        document.getElementById('routing').innerHTML = '<tr><th>UUID</th><th>Address</th></tr>';
...
async function getHashTable(){
    ...
    if (response.status == 200){
        let result = await response.json();
        document.getElementById('hashTable').innerHTML = '<tr><th>UUID</th><th>Public key</th><th>Filename</th><th>Hash value</th><th>Request access</th></tr>';
        for (let [key, value] of Object.entries(result)){
